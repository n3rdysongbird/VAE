# -*- coding: utf-8 -*-
"""
Created on Mon Jul 16 17:45:56 2018
...
def train(network_architecture, learning_rate=0.001,
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            batch_xs = next_batch(batch_size, y_mc)[0]
...
#         n_hidden_gener_2=30, # 2nd layer decoder neurons
#         n_input=380, # MNIST data input (img shape: 28*28)
...
network_architecture = \
    dict(n_hidden_recog_1=5, # 1st layer encoder neurons
         ...
         n_input=381, # MNIST data input (img shape: 28*28)
...
VAEdf['Location'] = y_sample

ax = sns.lmplot( x="Dim 2", y="Dim 1", data=VAEdf, fit_reg=False, hue='Location', legend=True, legend_out = True)

#dummy coding
#
#plt.scatter(z_mu[:, 0], z_mu[:, 1], c=y_sample)
#
## WHAT THE F*** IS BELOW THIS LINE?! LOL. Not sure how to visualize.
#
#
#
#
...
#plt.scatter(z_mu[:, 0], z_mu[:, 1], c=y_sample)#, c=np.argmax(y_sample))
#plt.legend()
#plt.grid()
#
#
...
#        canvas[(nx-i-1)*380:(nx-i)*380, j*1:(j+1)*1] = x_mean[0].reshape(1,380)
#
